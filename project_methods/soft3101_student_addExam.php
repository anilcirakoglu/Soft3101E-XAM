...
$user_data=$login->check_login_student($_SESSION['mybook_userid']);

if (isset($_GET['find'])) {
    $find=addslashes($_GET['find']);
    $findcreator=addslashes($_GET['find_teacher']);
    $sql="select * from create_exams where exam_name='$find' and creator_name='$findcreator' limit 1";
    $db=new Database();
    $results=$db->read($sql);
...
if (is_array($results) || is_object($results))
{
    ...
    {
        include("soft3101_show_examsAsStudent.php");
    }
}
    ?>
</body>
