...
$ROW=false;
if (isset($_GET['id'])){
    $ROW=$delete_lesson->get_data_taker_lesson($_GET['id']);
    $ROWW=$delete_exam->get_data_taker_exam_bylessonid($_GET['id']);
  }else {
      $ERROR="No exam was found!";
  }
if ($_SERVER['REQUEST_METHOD']=="POST") {
    $delete_class=new AddLesson();
    $delete_class->save_delete_lesson_byStudent($_POST,$ROW['lessonid']);
    $delete_class_exam=new AddExam();
    $delete_class_exam->save_delete_exam_byStudent($_POST,$ROWW['lessonid']);
    if ($result == "") {
