...
$add_teacher=new AdminSet();
$ERROR="";
$ROWW=false;
if (isset($_GET['id'])){
    $ROWW=$add_teacher->get_id_preTeacher_byAdmin($_GET['id']);
  }else {
      $ERROR="No teacher was found!";
  }
if ($_SERVER['REQUEST_METHOD']=="POST") {
    $add_class=new AdminSet();
    $add_class->create_approved_teacher($_POST,$ROWW['admin_id'],$ROWW['userid'],$ROWW['username'],$ROWW['password'],$ROWW['email']);
    $delete_preteacher_class=new AdminSet();
    $delete_preteacher_class->save_delete_preTeacher_byAdmin($_POST,$ROWW['userid']);
    if ($result == "") {
...
echo"<p id='textbox'>Are you sure want to accept this teacher ?</p>";
